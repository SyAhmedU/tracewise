...
export const GOVERNMENT: WorkedExample[] = [
  { key: 'vao-community-certificate', label: 'Issuing a community certificate (e-Sevai)', domain: 'Government', region: 'Rural TN', emoji: '🏛',
    summary: 'A VAO from portal queue to village doorstep — paper village registers, evening visits, defensible judgment when records conflict.', build: vaoCommunityCertificate },
  { key: 'traffic-constable', label: 'A traffic constable on junction duty', domain: 'Government', region: 'Chennai, TN', emoji: '🚦',
    summary: 'A constable spotting violations, booking e-Challans, defusing arguments, and keeping a personal notebook of the shift\'s flow.', build: trafficConstable },
  { key: 'rto-clerk', label: 'Issuing a driving license at the RTO', domain: 'Government', region: 'Chennai, TN', emoji: '🪪',
    summary: 'An RTO clerk between Sarathi, biometric, test inspector and a counter queue made of "sir status please" enquiries.', build: rtoClerk },
  { key: 'sub-registrar-clerk', label: 'Registering a property at the SRO', domain: 'Government', region: 'Tamil Nadu', emoji: '📜',
    summary: 'An SRO clerk on STAR portal — guideline-value judgment, biometric, scanner, and a personal day-log faster than the portal queue view.', build: subRegistrarClerk },
  { key: 'sanitary-inspector', label: 'Ward round of a municipal sanitary inspector', domain: 'Government', region: 'Tamil Nadu', emoji: '🧹',
    summary: 'An SI between SHG workers, blocked drains, food premises and a Commissioner WhatsApp group read faster than the ULB portal.', build: sanitaryInspector },
  { key: 'blo', label: 'A BLO during electoral roll revision', domain: 'Government', region: 'Tamil Nadu', emoji: '🗳',
    summary: 'A BLO going door-to-door with a printed list, collecting Forms 6/7/8, fighting ERONet at night, and managing booth-agent pushback.', build: blo },
  { key: 'pds-dealer', label: 'Monthly PDS distribution at a ration shop', domain: 'Government', region: 'Tamil Nadu', emoji: '🍚',
    summary: 'A PDS dealer between ePOS biometric, scoop scale, OTP fallback for elderly thumbs, and a paper register kept as insurance.', build: pdsDealer },
];
